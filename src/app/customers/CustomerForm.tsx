...
interface Props {
  ...
  initial?: {
    ...
    taxId: string; notes: string; status: string; creditLimit: string
  }
}

export default function CustomerForm({ mode, orgId, userId, userName, orgName, customerId, initial }: Props) {
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .page-title{font-size:22px;font-weight:800;color:#1A1A20;letter-spacing:-0.4px;flex:1}
        ...
        .fi,.fta{width:100%;padding:13px 16px;background:rgba(0,0,0,0.03);border:1.5px solid rgba(0,0,0,0.07);border-radius:14px;font-size:15px;font-weight:500;color:#1A1A20;font-family:inherit;outline:none;box-shadow:inset 2px 2px 6px rgba(0,0,0,0.06),inset -2px -2px 5px rgba(255,255,255,0.80);transition:border-color 0.15s}
        .fi:focus,.fta:focus{border-color:#2563EB;box-shadow:inset 2px 2px 6px rgba(0,0,0,0.06),inset -2px -2px 5px rgba(255,255,255,0.80),0 0 0 3px rgba(37,99,235,0.12)}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="card">
                <div className="field">
                  ...
                  <input className="fi" type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Ej. María González López" autoFocus autoComplete="name" />
                </div>
                <div className="field">
                  <div className="fl">Teléfono</div>
                  <input className="fi" type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+52 55 1234 5678" autoComplete="tel" />
                </div>
                <div className="field">
                  <div className="fl">Correo electrónico</div>
                  <input className="fi" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="cliente@ejemplo.com" autoComplete="email" />
                </div>
                <div className="field">
                  <div className="fl">RFC / ID fiscal (opcional)</div>
                  <input className="fi" type="text" value={taxId} onChange={e => setTaxId(e.target.value.toUpperCase())} placeholder="Ej. GOGM901012AB1" autoComplete="off" />
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div className="field">
                  ...
                  <input className="fi" type="number" min="0" step="0.01" value={creditLimit} onChange={e => setCreditLimit(e.target.value)} placeholder="0.00" />
                </div>
              </div>
              ...
              <div className="card">
                <div className="field">
                  <div className="fl">Notas internas</div>
                  <textarea className="fta" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Preferencias, historial, notas importantes..." />
                </div>
              </div>
