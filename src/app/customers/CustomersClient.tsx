    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat{background:#ECEEF2;border-radius:18px;padding:14px 16px;box-shadow:5px 5px 14px rgba(0,0,0,0.08),-3px -3px 10px rgba(255,255,255,0.95);cursor:pointer;border:2px solid transparent;transition:all 0.15s}
        .stat.on{border-color:#2563EB;box-shadow:0 4px 16px rgba(37,99,235,0.18)}
        .stat-num{font-size:26px;font-weight:800;color:#1A1A20;letter-spacing:-0.5px}
        .stat-lbl{font-size:11px;font-weight:600;color:rgba(26,26,32,0.40);text-transform:uppercase;letter-spacing:0.05em;margin-top:2px}
        .stat.on .stat-num{color:#1D4ED8}
        .stat.on .stat-lbl{color:#2563EB}
        ...
        .cust-row{display:flex;align-items:center;gap:12px;padding:13px 18px;border-top:1px solid rgba(0,0,0,0.05);text-decoration:none;transition:background 0.12s}
        ...
        .cust-name{font-size:14px;font-weight:700;color:#1A1A20}
        .cust-meta{font-size:12px;color:rgba(26,26,32,0.40);margin-top:2px}
        ...
        .balance{font-size:13px;font-weight:700;color:#1A1A20;margin-top:3px}
        .balance.owing{color:#DC2626}

        .empty{padding:48px 20px;text-align:center}
        .empty-icon{width:56px;height:56px;background:#ECEEF2;border-radius:18px;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;box-shadow:4px 4px 12px rgba(0,0,0,0.07),-3px -3px 8px rgba(255,255,255,0.90)}
        .empty-title{font-size:15px;font-weight:700;color:#1A1A20;margin-bottom:4px}
        .empty-sub{font-size:13px;color:rgba(26,26,32,0.38)}
        .count{font-size:12px;color:rgba(26,26,32,0.32);font-weight:500;margin-bottom:10px}
      `}</style>

      <div className="shell">
        <Sidebar orgName={orgName} userName={userName} active="customers" />
        <main className="main">
          ...
          <div className="content">
            ...
            <div className="search-wrap">
              <div className="search-icon">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(26,26,32,0.35)" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              </div>
              ...
              {filtered.length === 0 ? (
                <div className="empty">
                  <div className="empty-icon">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(29,78,216,0.50)" strokeWidth="1.8" strokeLinecap="round"><circle cx="9" cy="7" r="4"/><path d="M3 21a6 6 0 0 1 12 0"/><circle cx="17" cy="10" r="3"/><path d="M21 21a4 4 0 0 0-6 0"/></svg>
                  </div>
                ...
                return (
                  <Link key={c.id} href={`/customers/${c.id}`} className="cust-row">
                    ...
                    <div className="cust-right">
                      ...
                      {c.balance_owing > 0 && (
                        <div className="balance owing">-${c.balance_owing.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</div>
                      )}
                    </div>
                  </Link>
                )
