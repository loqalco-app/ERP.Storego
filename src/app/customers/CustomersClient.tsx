...
function fmtMoney(n: number) { return '$' + n.toLocaleString('es-MX', { minimumFractionDigits: 2 }) }
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat.on{border-color:var(--brand-mid,#2563EB);box-shadow:0 4px 16px rgba(37,99,235,0.18)}
        ...
        .search-input{width:100%;padding:12px 16px 12px 42px;background:var(--bg,#ECEEF2);border:1.5px solid rgba(0,0,0,0.07);border-radius:16px;font-size:14px;font-weight:500;color:var(--text-1,#1A1A20);font-family:inherit;outline:none}
        .search-input::placeholder{color:rgba(26,26,32,0.28)}
        .search-input:focus{border-color:#2563EB}
        .card{background:var(--bg,#ECEEF2);border-radius:24px;overflow:hidden;box-shadow:var(--shadow-card)}
        .cust-row{display:flex;align-items:center;gap:12px;padding:13px 18px;border-top:1px solid rgba(0,0,0,0.04);text-decoration:none;transition:background 0.12s}
        .cust-row:first-child{border-top:none}
        .cust-row:hover{background:rgba(29,78,216,0.04)}
        .cust-row:active{background:rgba(29,78,216,0.09)}
        ...
        .cust-right{text-align:right;flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:4px}
        .badge{display:inline-block;padding:3px 9px;border-radius:50px;font-size:10px;font-weight:700}
        .balance{font-size:12px;font-weight:700;color:#DC2626}
        .arrow{color:rgba(26,26,32,0.22);flex-shrink:0}
        .empty{padding:48px 20px;text-align:center}
        .empty-icon{width:56px;height:56px;background:var(--bg,#ECEEF2);border-radius:20px;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;box-shadow:var(--shadow-sm)}
            ...
            return (
              <Link key={c.id} href={"/customers/" + c.id} className="cust-row">
                ...
                <div className="cust-right">
                  ...
                  {c.balance_owing > 0 && <span className="balance">-{fmtMoney(c.balance_owing)}</span>}
                </div>
                <div className="arrow">
