...
export default function CustomersClient({ customers: initCustomers }: Props) {
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* ── Preview sheet ─────────────────────────────────────── */
        .sheet-backdrop{position:fixed;inset:0;z-index:400;background:rgba(0,0,0,0);transition:background 0.25s;pointer-events:none}
        .sheet-backdrop.open{background:rgba(0,0,0,0.40);pointer-events:all}
        .sheet{position:fixed;bottom:0;left:0;right:0;z-index:401;background:var(--bg,#ECEEF2);border-radius:24px 24px 0 0;padding:0 0 calc(env(safe-area-inset-bottom,0px) + 24px);box-shadow:0 -8px 40px rgba(0,0,0,0.14);transform:translateY(100%);transition:transform 0.28s cubic-bezier(0.32,0.72,0,1);max-height:90dvh;overflow-y:auto}
        .sheet.open{transform:translateY(0)}
        @media(min-width:768px){
          .sheet{left:auto;right:24px;bottom:24px;width:380px;border-radius:24px;max-height:calc(100dvh - 48px)}
        }
        .sheet-handle{width:36px;height:4px;border-radius:2px;background:rgba(0,0,0,0.15);margin:12px auto 0}
        .sheet-head{padding:20px 20px 16px;border-bottom:1px solid rgba(0,0,0,0.06)}
        .sheet-avatar{width:64px;height:64px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;color:white;margin-bottom:12px}
        ...
        .sheet-actions{display:flex;gap:10px;padding:16px 20px 4px}
        .sheet-close-btn{flex:1;padding:13px;border-radius:14px;border:none;background:rgba(0,0,0,0.06);color:var(--text-1,#1A1A20);font-size:14px;font-weight:700;font-family:inherit;cursor:pointer}
        .sheet-edit-btn{flex:2;padding:13px;border-radius:14px;border:none;background:linear-gradient(145deg,#1D4ED8,#2563EB);color:white;font-size:14px;font-weight:700;font-family:inherit;cursor:pointer;text-align:center;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:6px;box-shadow:0 4px 16px rgba(29,78,216,0.28)}
      `}</style>
      ...
      <div className="content">
        ...
        <div className="card">
          {filtered.length === 0 ? (
            ...
            return (
              <div key={c.id} className="cust-row" onClick={() => setPreview(c)}>
      ...
      {/* Preview sheet */}
      <div className={`sheet-backdrop${preview ? ' open' : ''}`} onClick={() => setPreview(null)} />
      {preview && (() => {
        const st = STATUS_STYLE[preview.status] ?? STATUS_STYLE.active
        return (
          <div className={`sheet${preview ? ' open' : ''}`}>
            ...
            <div className="sheet-body">
                ...
                <div className="sheet-num-row">
                  <div className="sheet-num-card">
                    <div className={`sheet-num-val${preview.balance_owing > 0 ? ' red' : ''}`}>
                      ${preview.balance_owing.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                    </div>
                    <div className="sheet-num-lbl">Saldo pendiente</div>
                  </div>
                  <div className="sheet-num-card">
                    <div className="sheet-num-val">
                      ${preview.credit_limit.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                    </div>
              ...
              <div className="sheet-section">
                ...
                <div className="sheet-row">
                  ...
                  <span className="sheet-val">Cliente desde {fmt(preview.created_at)}</span>
                </div>
              </div>
            </div>

            <div className="sheet-actions">
              <button className="sheet-close-btn" onClick={() => setPreview(null)}>Cerrar</button>
