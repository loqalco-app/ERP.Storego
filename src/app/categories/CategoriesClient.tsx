...
interface Category { id: string; name: string; slug: string; description: string | null; parent_id: string | null; productCount: number }
interface Props { categories: Category[]; orgId: string; userName: string; orgName: string }

function slugify(s: string) {
  return s.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
}

export default function CategoriesClient({ categories: initial, orgId, userName, orgName }: Props) {
  ...
  async function handleSave() {
    ...
    if (editing) {
      const { error } = await supabase.from('categories').update({ name: name.trim(), description: desc.trim() || null }).eq('id', editing.id)
      if (error) { setSaving(false); setErr(error.message); return }
      setCategories(cs => cs.map(c => c.id === editing.id ? { ...c, name: name.trim(), description: desc.trim() || null } : c))
    } else {
      ...
      setCategories(cs => [...cs, { ...data, productCount: 0 }])
    }
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-save { flex: 2; padding: 14px; border-radius: 14px; border: none; background: linear-gradient(145deg,#1D4ED8,#2563EB); font-size: 15px; font-weight: 700; color: white; cursor: pointer; font-family: inherit; box-shadow: 0 6px 18px rgba(29,78,216,0.28); transition: opacity 0.15s; }
        .btn-save:disabled { opacity: 0.5; cursor: not-allowed; }
      `}</style>

      <div className="shell">
        <Sidebar orgName={orgName} userName={userName} active="categories" />
        <main className="main">
          ...
          <div className="content">
            <div className="card">
              {categories.length === 0 ? (
                <div className="empty">Sin categorías — crea la primera para organizar tus productos</div>
              ) : categories.map(c => (
                <div key={c.id} className="row">
                  <div className="row-icon">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(29,78,216,0.55)" strokeWidth="1.8" strokeLinecap="round"><rect x="2" y="3" width="9" height="9" rx="2"/><rect x="13" y="3" width="9" height="9" rx="2"/><rect x="2" y="14" width="9" height="9" rx="2"/><rect x="13" y="14" width="9" height="9" rx="2"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="row-name">{c.name}</div>
                    <div className="row-sub">{c.productCount} producto{c.productCount !== 1 ? 's' : ''}{c.description ? ` · ${c.description}` : ''}</div>
                  </div>
                  <div className="row-actions">
                    <button className="act-btn act-edit" onClick={() => openEdit(c)}>Editar</button>
                    <button className="act-btn act-del" onClick={() => handleDelete(c.id)} disabled={deleting === c.id}>
      ...
      {showForm && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && closeForm()}>
          <div className="modal">
            ...
            <input className="field-input" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ej. Ropa, Electrónicos, Calzado..." autoFocus />
