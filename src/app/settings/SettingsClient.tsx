...
export default function SettingsClient({
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    setInvMsg({ type:'ok', text: data.method === 'resend' ? `Invitación enviada a ${invEmail}` : `Invitación creada sin correo (agrega RESEND_API_KEY).` })
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* topbar — solo ajusta padding-bottom */
        .topbar{padding-bottom:8px}
        .back-btn{width:38px;height:38px;border-radius:var(--r-sm,12px);background:var(--bg,#ECEEF2);display:flex;align-items:center;justify-content:center;text-decoration:none;box-shadow:var(--shadow-sm);flex-shrink:0}
        ...
        @media(min-width:768px){.content{padding-left:40px;padding-right:40px;padding-bottom:calc(var(--nav-h,88px) + 16px);max-width:680px}}

        /* tab bar de ajustes */
        .s-tabs{display:flex;gap:6px;padding:12px 20px 0;overflow-x:auto;-webkit-overflow-scrolling:touch}
        @media(min-width:768px){.s-tabs{padding:12px 40px 0}}
        .s-tab{padding:9px 18px;border-radius:var(--r-pill,50px);border:none;background:var(--bg,#ECEEF2);font-size:13px;font-weight:700;color:var(--text-3,rgba(26,26,32,0.45));cursor:pointer;font-family:inherit;box-shadow:var(--shadow-sm);transition:all 0.15s;white-space:nowrap;-webkit-tap-highlight-color:transparent}
        .s-tab.on{background:var(--brand-alpha,rgba(29,78,216,0.10));color:var(--brand,#1D4ED8);box-shadow:none}
        .s-tab-sep{width:1px;background:var(--border-light,rgba(0,0,0,0.07));margin:4px 0}

        /* avatar */
        .av-section{display:flex;align-items:center;gap:20px;margin-bottom:28px}
        .av-lg{width:72px;height:72px;border-radius:50%;background:var(--grad-brand);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:800;color:white;flex-shrink:0;box-shadow:var(--shadow-brand-sm)}

        /* form */
        .sec-title{font-size:17px;font-weight:800;color:var(--text-1,#1A1A20);letter-spacing:-0.3px;margin:20px 0 10px}
        ...
        .fi:focus{border-color:var(--brand-mid,#2563EB);box-shadow:var(--shadow-inset),0 0 0 3px var(--brand-alpha)}
        ...
        .hint{font-size:11px;color:var(--text-4);margin-top:5px;line-height:1.4}
        ...
        .save-btn{width:100%;padding:16px;background:var(--grad-brand-btn);border:none;border-radius:var(--r-xl,24px);font-size:16px;font-weight:700;color:white;cursor:pointer;font-family:inherit;box-shadow:var(--shadow-brand);transition:opacity 0.15s,transform 0.12s;margin-top:8px}
        .save-btn:hover{opacity:.92}
        .save-btn:active{transform:scale(0.98)}
        .save-btn:disabled{opacity:.5;cursor:not-allowed}
        .sec-btn{width:100%;padding:14px;background:rgba(0,0,0,0.05);border:none;border-radius:var(--r-xl,24px);font-size:15px;font-weight:700;color:var(--text-2,#0A0A0E);cursor:pointer;font-family:inherit;transition:background 0.15s,transform 0.12s;margin-top:8px}
        .sec-btn:hover{background:rgba(0,0,0,0.09)}
        ...
        .section-title{font-size:13px;font-weight:700;color:var(--text-3);text-transform:uppercase;letter-spacing:0.07em}
        .invite-btn{display:flex;align-items:center;justify-content:center;gap:6px;width:40px;height:40px;background:var(--grad-brand-btn);color:white;border:none;border-radius:50%;cursor:pointer;font-family:inherit;box-shadow:var(--shadow-brand-sm);transition:opacity 0.15s,transform 0.12s;flex-shrink:0}
        .invite-btn:hover{opacity:.90}
        .invite-btn:active{transform:scale(0.93)}
        @media(min-width:480px){.invite-btn{width:auto;height:auto;border-radius:var(--r-pill,50px);padding:9px 18px}}
        ...
        .av{width:40px;height:40px;border-radius:50%;background:var(--grad-brand);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:white;flex-shrink:0}
        ...
        .modules-chip{display:flex;flex-wrap:wrap;gap:5px;margin-top:5px}
        .mod{padding:2px 8px;border-radius:var(--r-pill,50px);background:rgba(0,0,0,0.05);font-size:10px;font-weight:600;color:var(--text-3)}
        ...
        .mfi{width:100%;padding:13px 16px;background:rgba(0,0,0,0.03);border:1.5px solid var(--border,rgba(0,0,0,0.07));border-radius:var(--r-md,16px);font-size:15px;font-weight:500;color:var(--text-1);font-family:inherit;outline:none;transition:border-color 0.15s;margin-bottom:14px}
        .mfi:focus{border-color:var(--brand-mid)}
        .role-pills{display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap}
        .rpill{flex:1;min-width:80px;padding:10px 8px;border-radius:var(--r-md,16px);border:1.5px solid var(--border,rgba(0,0,0,0.08));background:var(--bg);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.15s;text-align:center}
        .rpill.on{border-color:var(--brand);background:var(--brand-alpha);color:var(--brand)}
        ...
        .btn-cancel{flex:1;padding:14px;border-radius:var(--r-xl,24px);border:1.5px solid var(--border,rgba(0,0,0,0.10));background:transparent;font-size:15px;font-weight:700;color:var(--text-2);cursor:pointer;font-family:inherit}
        .btn-send{flex:2;padding:14px;border-radius:var(--r-xl,24px);border:none;background:var(--grad-brand-btn);font-size:15px;font-weight:700;color:white;cursor:pointer;font-family:inherit;box-shadow:var(--shadow-brand);transition:opacity 0.15s}
      ...
      {/* ── Tab switcher (dentro del topbar sticky) ── */}
      <div className="s-tabs">
        <button className={`s-tab${tab === 'profile' ? ' on' : ''}`} onClick={() => setTab('profile')}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{marginRight:6,verticalAlign:'middle'}}><circle cx="12" cy="8" r="4"/><path d="M4 20a8 8 0 0 1 16 0"/></svg>
          Mi perfil
        </button>
        <button className={`s-tab${tab === 'team' ? ' on' : ''}`} onClick={() => setTab('team')}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{marginRight:6,verticalAlign:'middle'}}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
          Equipo
        </button>
      </div>
        ...
        {tab === 'profile' && (
          <>
            <div className="av-section">
              <div className="av-lg">{initials(name || email)}</div>
              <div>
                <div style={{ fontSize:18, fontWeight:800, color:'var(--text-1,#1A1A20)' }}>{name || 'Sin nombre'}</div>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="card">
                ...
                <div className="field"><div className="fl">Teléfono</div><input className="fi" type="tel" value={tel} onChange={e => setTel(e.target.value)} placeholder="+52 55 1234 5678" autoComplete="tel" /></div>
              </div>

              <div className="sec-title">Correo electrónico</div>
              <div className="card">
                <div className="field">
                  ...
                  {newEmail !== email && <div className="hint">Se enviará confirmación a la nueva dirección.</div>}
                </div>
              </div>
              ...
              <div className="card">
                <div className="field">
                  ...
                  <div className="fi-wrap">
                    ...
                    <button type="button" className="eye-btn" onClick={() => setShowCur(v => !v)}><Eye show={showCur} /></button>
                  </div>
                </div>
                <div className="field">
                  ...
                  <div className="fi-wrap">
                    ...
                    <button type="button" className="eye-btn" onClick={() => setShowNew(v => !v)}><Eye show={showNew} /></button>
                  </div>
                  {newPass.length > 0 && (
                ...
                <div className="field">
                  ...
                  <div className="fi-wrap">
                    ...
                    <button type="button" className="eye-btn" onClick={() => setShowConf(v => !v)}><Eye show={showConf} /></button>
                  </div>
                </div>
              </div>
              <button type="submit" className="sec-btn" disabled={savingPass || !curPass || !newPass}>
        ...
        <div className="modal-scrim" onClick={e => { if (e.target === e.currentTarget) setShowInvite(false) }}>
          <div className="modal">
            ...
            <form onSubmit={handleInvite}>
              <div className="fl">Nombre y Apellido</div>
              <div className="name-row">
                <input className="mfi" style={{marginBottom:0}} type="text" placeholder="Nombre *" value={invFirstName} onChange={e => setInvFirstName(e.target.value)} autoFocus required />
                <input className="mfi" style={{marginBottom:0}} type="text" placeholder="Apellido" value={invLastName} onChange={e => setInvLastName(e.target.value)} />
              </div>
