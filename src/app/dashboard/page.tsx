...
// Cache dashboard stats per org for 60 seconds — the heavy counting queries
// Auth (getUser) stays uncached since it's the security gate
function getDashboardStats(orgId: string) {
    ...
    async () => {
      ...
      return {
        stats: {
          ...
          monthRevenue: (monthOrders ?? []).reduce((s: number, o: { total: string }) => s + Number(o.total), 0),
        },
...
export default async function DashboardPage() {
  ...
  const { stats, recentOrders, recentCustomers } = await getDashboardStats(orgId)
